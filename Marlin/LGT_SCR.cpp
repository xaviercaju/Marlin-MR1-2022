...
char steps_mm_X[8], steps_mm_Y[8],steps_mm_Z[8], steps_unit_E[8], set_new_mm[8], nuevos_pasos[8], mensaje_comando='M92 E', mensaje_mm[12], Comando_milimetros[12] ;// ajout Bruno + minifake

double longitud_mm=10.000;
...
//unsigned double longitud_mm = 10;
...
//dtostrf(planner.axis_steps_per_mm[3], 8, 3, steps_unit_E); 
...
/////////////////////     
//FUNCION MBL Y MOVIMIENTO DE Z 0.05mm
///////////////////
  ...
  case eBT_MBL_START:      
     ...
     break;
       ...
       if (mbl_count==(GRID_MAX_POINTS_Y * GRID_MAX_POINTS_X )) { // si tous les points sont fait 
            ...
            LGT_Change_Page(ID_DIALOG_MBL_FINISH);  // Cargar pagina confirmación
       }else{
...
////////// START MINIFAKE SET EXTRUDER STEPS //////////

case eBT_ESTEPS_CALENTAR:         //0049 button minifake
          
          menu_type = eMENU_ESTEPS_SET;
          thermalManager.setTargetHotend(PLA_E_TEMP, target_extruder);
...
case eBT_ESTEPS_EXTRUIR_100:  //0048 button   

enqueue_and_echo_commands_P(PSTR("G1 X50"));
         
        if (thermalManager.current_temperature[0] >= LGT_Get_Extrude_Temp())
      {
        current_position[E_AXIS] = current_position[E_AXIS]+100;
        LGT_Line_To_Current(E_AXIS);
      }
      else
      {
...
case eBT_ESTEPS_PASOS_SUBIR_10:                // Button - 004A 
      planner.axis_steps_per_mm[3]=planner.axis_steps_per_mm[3]+10;
break;  


case  eBT_ESTEPS_PASOS_BAJAR_10:               //Button - 004B
       planner.axis_steps_per_mm[3]=planner.axis_steps_per_mm[3]-10;
break;  


case  eBT_ESTEPS_PASOS_SUBIR_1:                //Button - 004C
       planner.axis_steps_per_mm[3]=planner.axis_steps_per_mm[3]+1;
break; 

 
case  eBT_ESTEPS_PASOS_BAJAR_1:               //Button - 004D
      planner.axis_steps_per_mm[3]=planner.axis_steps_per_mm[3]-1;
break;  


case  eBT_ESTEPS_PASOS_SUBIR_0:              //Button 75 - 004E
      planner.axis_steps_per_mm[3]=planner.axis_steps_per_mm[3]+0.1;
break;

  
case  eBT_ESTEPS_PASOS_BAJAR_0:                //Button - 004F
      planner.axis_steps_per_mm[3]=planner.axis_steps_per_mm[3]-0.1;
break;  


case  eBT_ESTEPS_PASOS_GUARDAR:                     //Button - 0050
      enqueue_and_echo_commands_P(PSTR("M500"));
      thermalManager.setTargetHotend(0, target_extruder); 
      menu_type = eMENU_HOME;
      LGT_Change_Page(ID_MENU_HOME);
      enqueue_and_echo_commands_P(PSTR("G28 X0 Y0 Z10"));      
break;  


 ///////END MINIFAKE FUNCTIONS SET EXTRUDER STEPS
...
*/
 ...
 /*case eBT_ESTEPS_SET_UP_1:

LGT_Change_Page(ID_MENU_ESTEPS);
          menu_type = eMENU_ESTEPS_SET; 
...
/////////// SAVE     enqueue_and_echo_commands_P(PSTR("M92 E97"));
    ////////////     enqueue_and_echo_commands_P(PSTR("M500"));
...
void LGT_SCR::LGT_Printer_Data_Updata()
{
  ...
  case eMENU_ESTEPS_SET:

    dtostrf(planner.axis_steps_per_mm[3], 8, 3, steps_unit_E);
    LGT_Send_Data_To_Screen1(ADDR_TXT_SETTINGS_StepPerUnit_E,steps_unit_E);
    //LGT_Send_Data_To_Screen1(ADDR_NEW_ESTEPS,Resultat_E);
      LGT_Send_Data_To_Screen(ADDR_VAL_TAR_E, thermalManager.target_temperature[0]);
      LGT_Send_Data_To_Screen(ADDR_VAL_CUR_E, (int16_t)thermalManager.current_temperature[0]);
    LGT_Send_Data_To_Screen(ADDR_VAL_MOVE_POS_X, (int16_t)(current_position[X_AXIS] * 10));
    LGT_Send_Data_To_Screen(ADDR_VAL_MOVE_POS_Y, (int16_t)(current_position[Y_AXIS] * 10));
